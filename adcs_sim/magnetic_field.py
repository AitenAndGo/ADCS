...
# ----------------------------------------------
# This module computes the Earth's magnetic field vector at the satellite's position
# using the IGRF-13 model for high-fidelity field calculations. It also provides
# coordinate transformations between ECI, geodetic, and body frames.
#
# - Computes magnetic field vector at satellite position
# - Uses IGRF-13 model for accurate field calculations
# - Transforms between coordinate frames (ECI, NED, body)
# - Can be extended to support other geomagnetic models if needed
# ==============================================
...
# Initialize IGRF model (GeoMag library)
gm = geomag.GeoMag()

def cartesian_to_geodetic(position_eci):
    """
    Convert Cartesian ECI coordinates to geodetic coordinates (latitude, longitude, altitude).
    This is required for input to the IGRF model, which expects geodetic coordinates.

    Parameters:
        position_eci (array-like): [x, y, z] position vector in ECI frame (km)
...
def calculate_magnetic_field_igrf(position_eci, sim_time_seconds):
    """
    Calculate Earth's magnetic field at a given ECI position and simulation time using the IGRF-13 model.
    The field is returned in the ECI frame (Tesla).

    Parameters:
        position_eci (array-like): [x, y, z] position vector in ECI frame (km)
        sim_time_seconds (float): Simulation time in seconds from INITIAL_EPOCH

    Returns:
        magnetic_field_eci (np.ndarray): [Bx, By, Bz] magnetic field vector in ECI frame (Tesla)
    """
...
def get_magnetic_field_readings(position, quaternion, sim_time_seconds):
    """
    Calculate magnetic field readings in both the ECI and satellite body frames.
    This function is typically called by the simulation to provide the field
    as sensed by the satellite at its current position and attitude.

    Parameters:
        position (array-like): [x, y, z] position vector in ECI frame (km)
        quaternion (array-like): [q0, q1, q2, q3] rotation quaternion from ECI to body frame
        sim_time_seconds (float): Simulation time in seconds from INITIAL_EPOCH

    Returns:
        magnetic_field_eci (np.ndarray): [Bx, By, Bz] magnetic field vector in ECI frame (Tesla)
        magnetic_field_body (np.ndarray): [Bx, By, Bz] magnetic field vector in body frame (Tesla)
    """
